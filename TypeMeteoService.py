...
page.insert_text("Current temperature: "+ str(temp), 30, 26)
page.insert_line("-", 30)
page.insert_text("Next 3 days prevision :", 3, 32)

for i in range(0,3):
    page.insert_text(day_previsions[i].ascii_img,3, 34+i*7)
    
for i in range(0,3):
    day_prevision = day_previsions[i]
    page.insert_text(day_prevision.ascii_img,3, 34+i*7)    
    page.insert_text(day_prevision.datetime_day.strftime("%A the %d %B %Y"), 30, 37+i*7)
    page.insert_text("Weather type: "+ day_prevision.weather_type, 30, 38+i*7)
    page.insert_text("Temperature range: "+ str(day_prevision.temp_min)+ "-" + str(day_prevision.temp_max), 30, 39+i*7)
    page.insert_text("_"*30,30, 40+i*7)
